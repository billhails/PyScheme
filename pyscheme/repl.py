...
class Repl:
    def __init__(self, in_file: StringIO, out_file: StringIO, error_file: StringIO):
        self.input = in_file
        self.output = out_file
        self.error = error_file
        self.tokeniser = reader.Tokeniser(in_file)
        self.reader = reader.Reader(self.tokeniser, error_file)
        operators = {
                    ...
                    "genstr": expr.GenStr(),           # list(char) -> list(char)
                    "str": expr.Str(),                 # t -> list(char)
                    "back": expr.Back(),               # _
        ...
        self.compile_time_env.install(globalenv, self.compile_time_env)
        self.compile_time_env = self.compile_time_env.extend({})  # fresh empty env for top-level
    ...
    def trampoline(self, threads: List['types.Promise']):
        while len(threads) > 0:
            ...
            cont = thunk()
            if cont is not None:
                if type(cont) is list:  # spawn returns a list of two threads
                    threads += cont
                else:
                    threads += [cont]
    ...
    def analyze(self, expression: expr.Expr, ret: 'types.Continuation', amb: ambivalence.Amb) -> 'types.Promise':
        try:
            expression.analyse(self.type_env)
        except PySchemeError as e:
            self.error.write(str(e))
            return None
        return lambda: ret(expression, amb)

    def eval(self, expression: expr.Expr, ret: 'types.Continuation', amb: ambivalence.Amb) -> 'types.Promise':
        return lambda: expression.eval(self.env, ret, amb)
    ...
    def repl(self, amb: ambivalence.Amb) -> 'types.Promise':
        def print_continuation(_, amb: ambivalence.Amb) -> 'types.Promise':
